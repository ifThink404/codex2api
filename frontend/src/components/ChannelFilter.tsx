...
// 仪表盘/用量页共用的上游渠道过滤（全部/Codex/Grok/Antigravity）。
// 选择持久化到 localStorage，两页共享同一份状态键。
export type UsageChannel = "" | "codex" | "grok" | "antigravity";
...
export function useUsageChannel(): [UsageChannel, (next: UsageChannel) => void] {
  const [channel, setChannel] = useState<UsageChannel>(() => {
    try {
      ...
      if (raw === "codex" || raw === "grok" || raw === "antigravity") return raw;
    } catch {
      // ignore
    }
    return "";
