		...
		go func() {
			...
			for ctx.Err() == nil {
				...
				for _, row := range rows {
					...
					if !account.AutomaticProbesEnabled() || !proxy.CodexTurnStateTemplateRenewalDue(account, row, time.Now()) || !account.ModelCatalogEligible() {
						continue
					}
					select {
					case slots <- struct{}{}:
...
func (h *Handler) renewCodexTurnStateModel(ctx context.Context, row database.CodexTurnStateTemplate, now func() time.Time) {
	account := h.store.FindByID(row.AccountID)
	if ctx.Err() != nil || !account.AutomaticProbesEnabled() || !proxy.CodexTurnStateTemplateRenewalDue(account, row, now()) || !account.ModelCatalogEligible() {
		return
	}
	...
	// Renewal sends inference requests, so it shares the optional probe gate and
	// attempt clock. A custom account interval overrides the inherited retry delay;
	// unsuccessful attempts still consume that interval. Manual refresh is separate.
	if !account.TryBeginAutomaticProbe(codexTurnStateRenewalRetryInterval) {
		return
	}
	defer account.FinishUsageProbe()
	ctx = auth.WithAutomaticProbeReservation(ctx, account)
	...
	if routeErr == nil {
		...
		saved, probeErr = h.refreshAutomaticCodexTurnStateModel(ctx, account, row.Model, route.url)
	}
...
// Automatic renewal uses the same acquisition/verification primitives as manual
// refresh, but rechecks live policy between requests. A queued worker or acquired
// candidate must not authorize further probes after the operator switches off.
func (h *Handler) refreshAutomaticCodexTurnStateModel(parent context.Context, account *auth.Account, model, proxyURL string) (bool, error) {
	...
	allowed := func() error {
		...
		if !account.AutomaticProbesEnabled() || !account.ModelCatalogEligible() || !proxy.CodexTurnStateModelAllowed(account, model) {
			return fmt.Errorf("automatic turn-state renewal disabled by account policy")
		}
		return nil
	}
