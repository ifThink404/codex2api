...
func TestPromptFilterNewAPIBindingAdminLifecycleMasksSecretsAndReloadsStore(t *testing.T) {
	...
	createBody := `{"api_key_id":` + jsonNumber(apiKeyID) + `,"platform_code":"fanren","platform_name":"凡人 NewAPI","policy_mode":"enforce","policy_profile":"balanced"}`
	...
	patched := do(http.MethodPatch, "/api/admin/prompt-filter/newapi-bindings/"+jsonNumber(apiKeyID), `{"platform_name":"凡人生产站","enabled":false,"policy_mode":"warn","policy_profile":"strict"}`)
	...
	if runtimeBinding.PlatformName != "凡人生产站" || runtimeBinding.Enabled || runtimeBinding.PolicyMode != "warn" || runtimeBinding.PolicyProfile != "strict" {
...
func TestPromptFilterNewAPIBindingSecretRotationCompletesAfterClientCancellation(t *testing.T) {
	...
	if err := db.CreatePromptFilterNewAPIBinding(context.Background(), &database.PromptFilterNewAPIBinding{
		...
		Enabled: true, PolicyMode: database.PromptFilterPolicyModeEnforce, PolicyProfile: database.PromptFilterPolicyProfileBalanced,
	}); err != nil {
